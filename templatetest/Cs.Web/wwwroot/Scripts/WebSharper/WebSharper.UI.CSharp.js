...
{
 "use strict";
 var WebSharper,UI,ViewExtensions,DocExtension,Client,Helpers,Obj,RouteMapBuilder,RouteItemParsers,View,Doc,Var$1,List,Runtime,RouteMap,Seq,Unchecked,Arrays,Collections,Map,Nullable,Operators,FSharpMap,Option;
 WebSharper=Global.WebSharper=Global.WebSharper||{};
 UI=WebSharper.UI=WebSharper.UI||{};
 ViewExtensions=UI.ViewExtensions=UI.ViewExtensions||{};
 DocExtension=UI.DocExtension=UI.DocExtension||{};
 ...
 ViewExtensions.WithInit=function(v,a)
 {
  return View.WithInit(a,v);
 };
 ViewExtensions.UpdateWhile=function(va,a,vb)
 {
  return View.UpdateWhile(a,vb,va);
 };
 DocExtension.DocLens=function(model,f)
 {
  return Doc.Flatten(model.MapLens(f));
 };
 DocExtension.Doc=function(model,f)
 {
  return Doc.ConvertSeqBy(model.key,function($1)
  {
   return function($2)
   {
    return f($1,$2);
 ...
 DocExtension.Doc$1=function(model,f)
 {
  return Doc.ConvertBy(model.key,f,model["var"].get_View());
 };
 DocExtension.DocLens$1=function(v,f,g)
 {
  return Doc.ConvertSeqVarBy(f,g,v);
 };
 ...
 {
  return Var$1.Lens(l,List.ofSeq,function(a,b)
  {
   return b;
  });
 };
 ...
 RouteItemParsers.MakeLink=function(shape)
 {
  return function(value)
  {
   ...
   function m(name,queryItem,a,link)
   {
    ...
    {
     return $3.Add$1($1,$2);
    },p[1],map[0]),p[0])):Unchecked.Equals(queryItem,1)?(map[0]=(v=List.head((link(value[name]))[0]),map[0].Add$1(name,v)),List.T.Empty):Unchecked.Equals(queryItem,2)?(m$2=value[name],m$2!=null&&m$2.$==1?map[0]=(v$1=List.head((link(m$2.$0))[0]),map[0].Add$1(name,v$1)):void 0,List.T.Empty):Unchecked.Equals(queryItem,3)?(v$2=value[name],(v$2!=null?map[0]=(v$3=List.head((link(Nullable.get(v$2)))[0]),map[0].Add$1(name,v$3)):void 0,List.T.Empty)):Operators.FailWith("invalid QueryItem enum value");
   }
   function g(t)
   {
    return t[0];
